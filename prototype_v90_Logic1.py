...
class MyModel(Model):
    def __init__(self, network_size=128):
        ...
        self.d1 = Dense(network_size)
        self.relulayer = Lambda(tf.nn.relu)
        self.d2 = Dense(10)

    def call(self, x, node=-1):
        x = self.flatten(x)
        x = self.d1(x)
        if node is not 1:
            x = x.numpy()
...
class NetworkUnit:
    ...
    def __init__(self, _train_ds, _test_ds, _sample_weight_multiplier, _sample_weight_adjuster, network_size=128, _sample_weights=None):
        ...
        self.model = MyModel(network_size=network_size)
    ...
    def test_node(self, images, labels):
        pass
    ...
    def get_weights(self):
        return [self.model.flatten.weights, self.model.d1.weights, self.model.d2.weights]
...
def trial(_train_ds, _test_ds, sample_weight_multiplier, sample_weight_adjuster, thesis_epochs=1, antithesis_epochs=1, network_size=128, print_data=False):
    thesis_model = NetworkUnit(_train_ds, _test_ds, sample_weight_multiplier, sample_weight_adjuster, network_size=network_size)
    sample_weights = thesis_model.train(epochs=thesis_epochs)
    antithesis_model = NetworkUnit(_train_ds, _test_ds, sample_weight_multiplier, sample_weight_adjuster, network_size=network_size, _sample_weights=sample_weights)  #
    ...
    return thesis_model.model, antithesis_model.model, sample_weights, anti_sample_weights, thesis_model.test_accuracy.result()

...
CUTOFF = int(len(x_train)/4)  # Change the integer to adjust the portion of data cut
SAMPLE_WEIGHT = 1
...
# Model Description
class MyModel(Model):
    def __init__(self, _former_networks):
        super(MyModel, self).__init__()
        self.num_feeder_networks = len(_former_networks)
        self.feeder_flatteners = []
        self.feeder_networks = _former_networks
...
class NetworkUnit:
    ...
    def __init__(self, _former_networks, _sample_weights=None):
        self.model = MyModel(_former_networks)
...
def trial(former_networks=np.array([]), blind_spots=np.array([True]), epochs=1, print_data=False):
    binary_synthesis = NetworkUnit(former_networks)
    sample_weights = binary_synthesis.train(epochs)
    sample_errors = (sample_weights == SAMPLE_WEIGHT + 1)

    blind_spots = (blind_spots & sample_errors)

    print(f'Blind Spots: {len(sample_errors[blind_spots])}')

    return binary_synthesis.model, blind_spots

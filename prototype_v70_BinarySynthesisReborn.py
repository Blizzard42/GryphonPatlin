...
def trial(_train_ds, _test_ds, sample_weight_multiplier, sample_weight_adjuster, former_scores=[True], type=0,
          epochs=1, print_data=False):
    sample_weights_in = np.multiply(sample_weight_multiplier,
                                    former_scores) + sample_weight_adjuster if len(former_scores) > 1 else None
    thesis_network = NetworkUnit(_train_ds, _test_ds, sample_weight_multiplier, sample_weight_adjuster,
                                 _sample_weights=sample_weights_in)  #
    sample_weights_out = thesis_network.train(epochs=epochs)
    score = sample_weights_out == sample_weight_multiplier + sample_weight_adjuster
    if type == 0:
        print('Synthesis: ')
        former_scores = former_scores & score
    elif type == 1:
        print('Antithesis: ')
        former_scores = former_scores | score
    print(f'Total Errors: {len(former_scores[former_scores])}')
